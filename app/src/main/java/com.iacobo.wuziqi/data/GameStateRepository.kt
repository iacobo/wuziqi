...
class GameStateRepository(private val context: Context) {
     ...
     */
    suspend fun loadGameState(gameState: GameState): Boolean =
            withContext(Dispatchers.IO) {
    ...
    suspend fun clearSavedState() = withContext(Dispatchers.IO) { prefs.edit { remove(KEY_BOARD) } }
