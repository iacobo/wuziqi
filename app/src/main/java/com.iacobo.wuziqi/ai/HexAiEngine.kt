...
/**
 * A strong AI player for Hex using Monte Carlo Tree Search (MCTS) with
 * virtual connection analysis and strategic patterns.
 * 
 * Hex is a complex strategy game with a high branching factor, where
 * the first player has a theoretical winning strategy. This AI combines
 * several techniques to create a strong, challenging opponent:
 * 
...
class HexAIEngine(private val random: Random = Random) {
    ...
    companion object {
        ...
        private val POSITION_VALUES = mapOf(
            // Center has high value
            Pair(5, 5) to 3.0,
            // Adjacent to center
            Pair(4, 5) to 2.0, Pair(6, 5) to 2.0, Pair(5, 4) to 2.0, Pair(5, 6) to 2.0,
            Pair(4, 4) to 1.8, Pair(6, 6) to 1.8, Pair(4, 6) to 1.8, Pair(6, 4) to 1.8,
            // Edge positions are important in Hex
            Pair(0, 5) to 1.5, Pair(10, 5) to 1.5, Pair(5, 0) to 1.5, Pair(5, 10) to 1.5
        )
        ...
        private val HEX_PATTERNS = listOf(
            ...
            HexPattern(
                layout = arrayOf(
                    arrayOf(PLAYER_TWO, EMPTY, PLAYER_TWO)
                ),
                centerX = 1, centerY = 0,
                value = 2.0,
    ...
    /**
     * Respond to the first move by the opponent. 
     * In Hex, there are strategic responses to the opening move.
     */
    ...
    private fun runMonteCarloTreeSearch(gameState: GameState): Pair<Int, Int> {
        ...
        val rootNode = MCTSNode(null, null, PLAYER_ONE) // Player ONE just moved, now it's TWO's turn
        ...
        for (i in 0 until SIMULATION_COUNT) {
            ...
            val expandedNode = if (selectedNode.unexploredMoves.isNotEmpty()) {
    ...
    private fun evaluatePatterns(
        ...
        val patterns = HEX_PATTERNS.map { pattern ->
            if (player == PLAYER_ONE) {
                ...
                for (i in 0 until pattern.height) {
                    for (j in 0 until pattern.width) {
                        ...
                        if (rotatedLayout[j][i] == PLAYER_ONE) rotatedLayout[j][i] = PLAYER_TWO
                        else if (rotatedLayout[j][i] == PLAYER_TWO) rotatedLayout[j][i] = PLAYER_ONE
                    }
                }
        ...
        for (pattern in patterns) {
            ...
            if (startRow < 0 || startRow + patternHeight > boardSize ||
                startCol < 0 || startCol + patternWidth > boardSize) {
                continue
            }
    ...
    private fun findHorizontalConnection(
        ...
        return if (candidateMoves.isNotEmpty()) {
            ...
            val sortedMoves = candidateMoves.sortedByDescending { (_, col) -> 
    ...
    private fun findVerticalConnection(
        ...
        return if (candidateMoves.isNotEmpty()) {
            // Sort by progression toward the goal
            val sortedMoves = candidateMoves.sortedByDescending { (row, _) -> 
    ...
    private fun findConnectionPath(
        ...
        while (current != null && !(current.first == start.first && current.second == start.second)) {
            path.add(current)
