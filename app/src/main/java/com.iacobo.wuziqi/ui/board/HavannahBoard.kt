...
/**
 * Implementation of a Havannah game board. Havannah uses a hexagonal grid with a hexagonal shape
 * (vs. the parallelogram shape of Hex).
 */
            ...
            Canvas(
                            ...
                            Modifier.fillMaxSize().pointerInput(isGameFrozen) {
                                if (!isGameFrozen) {
                                    detectTapGestures { tapOffset ->
                                        ...
                                        val radius =
                                                minOf(boardWidthPx, boardHeightPx) /
                                                        (boardSize * 2.5f)
                                        ...
                                        for (q in -boardSize + 1 until boardSize) {
                                            for (r in -boardSize + 1 until boardSize) {
                                                ...
                                                if (distance < bestDistance) {
                                                    ...
                                                    // Transform from axial coordinates to array
                                                    // indices
                                        ...
                                        // close enough
                                        if (bestRow >= 0 &&
                                                        bestRow < gameState.boardSize &&
                                                        bestCol >= 0 &&
                                                        bestCol < gameState.boardSize &&
                ...
                for (q in -boardSize + 1 until boardSize) {
                    for (r in -boardSize + 1 until boardSize) {
                        // Skip invalid coordinates in hexagonal grid
                        val s = -q - r
                        if (kotlin.math.abs(q) + kotlin.math.abs(r) + kotlin.math.abs(s) >
                                        2 * (boardSize - 1)
                        ) {
                            continue
                        }
                ...
                if (winningPath.isNotEmpty()) {
                    ...
                    for (pos in winningPath) {
                        ...
                        val winType = if (winningPath.size <= 1) 0 else gameState.getWinningType()
