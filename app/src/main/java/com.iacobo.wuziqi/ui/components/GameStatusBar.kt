...
fun getPlayerColor(gameState: GameState, gameType: GameType, winner: Int?): Color {
    ...
    return when (gameType) {
        GameType.Connect4 -> {
            if (currentPlayer == GameState.PLAYER_ONE) Connect4PieceRed else Connect4PieceYellow
        }
        GameType.Hex -> {
        ...
        return when (gameType) {
            GameType.TicTacToe ->
                    stringResource(
                            ...
                            if (winner == GameState.PLAYER_ONE) stringResource(R.string.player_x)
                            else stringResource(R.string.player_o)
                    )
            GameType.Connect4 ->
                    stringResource(
                            R.string.winner_format,
                            if (winner == GameState.PLAYER_ONE) stringResource(R.string.player_red)
